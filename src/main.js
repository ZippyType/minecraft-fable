...
import { MainMenu, SKINS } from './ui/menu.js';
...
const menu = new MainMenu(menuRoot, (worldConfig) => {
  menu.hide();
  startGame(worldConfig);
});
menu.show();

function startGame(worldConfig) {
  ...
  const seed = worldConfig.seed || 20260703;
  const terrain = new Terrain(seed);
  const world = new World(scene, terrain, solidMat, waterMat);

  const save = new SaveManager(worldConfig.name);
  ...
  if (savedGame) {
    ...
    hud.refresh();
    hud.refreshSelectedName();
  } else if (worldConfig.mode === 'creative') {
